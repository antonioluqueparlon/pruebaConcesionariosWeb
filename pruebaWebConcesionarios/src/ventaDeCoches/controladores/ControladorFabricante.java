...
public class ControladorFabricante extends Controlador {

	private static ControladorFabricante controlador = null;

	public ControladorFabricante () {
		super(Fabricante.class, "BaseDatosSwing");
	}
	 ...
	 * @return
	 */
	public static ControladorFabricante getControlador () {
		if (controlador == null) {
			controlador = new ControladorFabricante();
		}
		return controlador;
	}
	...
	public List<Fabricante> findAllLimited (int limit, int offset) {
		EntityManager em = getEntityManagerFactory().createEntityManager();
		Query q = em.createQuery("SELECT c FROM Cliente c", Fabricante.class);
